...
- `/scanner` — Camera capture → AI nutrition estimate → add to log
...
- All state (today's log, weight history, water/dose/reminder, theme, units, goals) lives in `localStorage` under `avolens.state.v1` — there's no backend/database.
