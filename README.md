# AvoLens (React Native)

The native iOS/Android AvoLens calorie & macro tracker, built with Expo
(SDK 57) + expo-router + TypeScript. Originally a 1:1 port of the AvoLens
web app (since retired — this native app is now the project), with the same
design tokens (light & dark), Poppins / Plus Jakarta Sans type, and
client-side state model — plus native camera capture and real local
notifications.
...
If Supabase is **not** configured, the scanner can fall back to any server
exposing the old web app's `/api/scan` route — set
`EXPO_PUBLIC_API_URL=https://<your-server>` in `.env`. Without either
backend, the Scanner shows a clear error; barcode scanning and manual
entry still work.
